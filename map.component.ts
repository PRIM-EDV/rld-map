...
export class MapComponent {
    ...
    private addPan() {
        ...
        this.mc.on("pan", (e: any) => {
            ...
            if (e.maxPointers === 1 && !this.mapService.dragged) {
                this.sx = Math.max(Math.min(0, this.mx), Math.min(Math.max(0, this.mx), px - e.deltaX * this.zoom)); // Math.max(px - (e.deltaX * this.zoom), 0);
                this.sy = Math.max(Math.min(0, this.my), Math.min(Math.max(0, this.my), py - e.deltaY * this.zoom)); // Math.max(py - (e.deltaY * this.zoom), 0);
                this.updateMap();
            }
        });
    }
